...
my_dir = r'C:\Users\tfluhr\Desktop\pquest_joa'
...
for dirs, subs, files in os.walk(my_dir):
    for f in files:
        ...
        # merge these lists
        new_lst = [list(x) for x in zip(ids, titles, f_dois, f_keywords, f_su, f_thes, f_naics, f_unclass, f_date, f_volume, f_issue, f_fpage, f_pcount, f_dtype, f_author, f_affil, f_cited)]
        ...
        df = DataFrame(new_lst, columns=['article id', 'title', 'doi', 'keywords', 'subjects', 'thesaurus', 'naics', 'unclass', 'f_date', 'f_volume', 'f_issue', 'f_page', 'f_pcount', 'f_dtype', 'f_author', 'f_affil', 'f_cited'])    
        
        #print(df)
        f = f[:-4]
        final_path = 'C:\\Users\\tfluhr\\Desktop\\jm_results\\' + f + '.csv'
