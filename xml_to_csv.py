...
my_dir = r'C:\Users\tfluhr\Desktop\new_pq_jm'
...
for dirs, subs, files in os.walk(my_dir):
    for f in files:
        ...
        # merge these lists
        new_lst = [list(x) for x in zip(ids, titles, f_dois, f_keywords, f_su, f_thes, f_naics, f_unclass, f_date, f_volume, f_issue, f_fpage, f_pcount, f_dtype, f_author)]
        ...
        df = DataFrame(new_lst, columns=['article id', 'title', 'doi', 'keywords', 'subjects', 'thesaurus', 'naics', 'unclass', 'f_date', 'f_volume', 'f_issue', 'f_page', 'f_pcount', 'f_dtype', 'f_author'])    
